...
NSNotificationName const WKUnitFieldDidBecomeFirstResponderNotification = @"WLUnitFieldDidBecomeFirstResponderNotification";
NSNotificationName const WKUnitFieldDidResignFirstResponderNotification = @"WLUnitFieldDidResignFirstResponderNotification";
#else
NSString *const WKUnitFieldDidBecomeFirstResponderNotification = @"WLUnitFieldDidBecomeFirstResponderNotification";
NSString *const WKUnitFieldDidResignFirstResponderNotification = @"WLUnitFieldDidResignFirstResponderNotification";
#endif

@interface WKPwdTextField () <UIKeyInput>
...
- (void)initialize {
    ...
    _defaultKeyboardType = WKKeyboardTypeNumberPad;
...
- (BOOL)becomeFirstResponder {
    ...
    if (result ==  YES) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:WKUnitFieldDidBecomeFirstResponderNotification object:nil];
    }
...
- (BOOL)resignFirstResponder {
    ...
    if (result) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:WKUnitFieldDidResignFirstResponderNotification object:nil];
    }
...
- (void)insertText:(NSString *)text {
    ...
    if ([self.delegate respondsToSelector:@selector(WKunitField:shouldChangeCharactersInRange:replacementString:)]) {
        if ([self.delegate WKunitField:self shouldChangeCharactersInRange:NSMakeRange(_string.count - 1, 1) replacementString:text] == NO) {
            return;
        }
    }
...
- (void)deleteBackward {
    ...
    if ([self.delegate respondsToSelector:@selector(WKunitField:shouldChangeCharactersInRange:replacementString:)]) {
        if ([self.delegate WKunitField:self shouldChangeCharactersInRange:NSMakeRange(_string.count - 1, 0) replacementString:@""] == NO) {
            return;
        }
    }
...
- (UIKeyboardType)keyboardType {
    if (_defaultKeyboardType == WKKeyboardTypeASCIICapable) {
