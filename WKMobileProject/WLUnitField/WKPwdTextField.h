 ...
 */
typedef NS_ENUM(NSUInteger, WKKeyboardType) {
    WKKeyboardTypeNumberPad,
    WKKeyboardTypeASCIICapable
};

@protocol WKPwdTextFieldDelegate;

IB_DESIGNABLE

@interface WKPwdTextField : UIControl

@property (nullable, nonatomic, weak) id<WKPwdTextFieldDelegate> delegate;
/**
 保留的用户输入的字符串
 */
...
#else
@property (nonatomic, assign) WKKeyboardType defaultKeyboardType;       // 默认为 WLKeyboardTypeNumberPad。
...
@protocol WKPwdTextFieldDelegate <NSObject>
