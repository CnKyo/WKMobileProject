...
@interface WKPasswordTextFiled ()<UITextFieldDelegate>
@property (nonatomic, strong) NSMutableArray *textFields;
@property (nonatomic, assign) NSInteger index;
@end

@implementation WKPasswordTextFiled
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField{
    if (![textField.superview isKindOfClass:[WKPasswordTextFiled class]]) {
        _index ++;
...
- (void)deleteBtn{
    ...
    if(![self.superview isKindOfClass:[WKPasswordTextFiled class]]){
        if (textField.text.length > 0) {
            textField.text = [textField.text substringToIndex:textField.text.length - 1];
        }
        return;
    }
    WKPasswordTextFiled *inputView = (WKPasswordTextFiled *)textField.superview;
