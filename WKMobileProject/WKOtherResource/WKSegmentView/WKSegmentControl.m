...
- (void)loadArrWithView:(NSArray *)mArray andType:(int)mType{
    ...
    for (int i = 0; i < mArray.count; i++) {
        ...
        if (mType == 2) {
            ...
            [btn setBackgroundImage:[UIImage imageNamed:@"icon_segment_unselected"] forState:UIControlStateNormal];
            [btn setBackgroundImage:[UIImage imageNamed:@"icon_segment_selected"] forState:UIControlStateSelected];
        ...
        for (int j = 1; j<3; j++) {
            UIView  *mline3 = [UIView new];
            mline3.backgroundColor = [UIColor colorWithRed:0.972549019607843 green:0.584313725490196 blue:0.270588235294118 alpha:1.00];
            mline3.frame = CGRectMake(mBtnWith*j, 15, 0.5, self.bounds.size.height-30);
