...
@property (weak, nonatomic) IBOutlet CountDownLabel *mCountLb;

@end

@implementation WKScanDeviceViewController
{
    ...
    __weak IBOutlet UIImageView *_scanPane;
}
- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.navigationItem.title = @"扫描设备二维码";
    [[ControlRunLoopTool sharedTool] setUseRunLoop:[ControlRunLoopTool sharedTool].useRunLoop];
    [_mCountLb setupCountDownWithTargetTime:[NSDate dateWithTimeIntervalSinceNow:900]];
...
//                MLLog(@"包含了：%@",mCurrentString);
                ...
                [para setObject:@"eHgyMDE3MDkwNTAyNDUxNzYwNzM4NDA4" forKey:@"device_barcode"];
                [MWBaseObj MWVeryfyDevice:para block:^(MWBaseObj *info) {
                    if (info.err_code == 0) {
                        [SVProgressHUD showSuccessWithStatus:info.err_msg];
                        [self popViewController];
                        self.block(2,@"eHgyMDE3MDkwNTAyNDUxNzYwNzM4NDA4");
                    }else{
